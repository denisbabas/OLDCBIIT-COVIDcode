...
public class ServicePortalSurveySteps extends PageInitializer {
	...
	@Given("a user has logged in using their PIV card")
	public void a_user_has_logged_in_using_their_PIV_card() throws TestingException {
		nativeViewLoginImpl.nativeViewLogin();
	}
	
	@Given("opens a COVIDCode Volunteer Survey record in the Native View")
	public void opens_a_COVIDCode_Volunteer_Survey_record_in_the_Native_View()  {
		nativeViewStepsImpl.opensCOVIDCodeVolunteerSurveyrecordIntheNativeView();
	}

	@Given("the How did you find out about our study? field is updated to {string}")
	public void the_How_did_you_find_out_about_our_study_field_is_updated_to(String otherSpecify) {
	    nativeViewStepsImpl.theHowDidYouFindOutAboutOurStudyFieldIsUpdated(otherSpecify);
	}
	    
	@Then("the {string} field is visible")
	public void the_field_is_visible(String pleaseSpecify) {
	    nativeViewStepsImpl.theFieldIsVisible(pleaseSpecify);
	}
